...
function detectType(lines) {
  if (lines.length >= 2 && lines[0].length === 44) return 'TD3_PASSPORT';
  if (lines.length >= 3 && lines[0].length === 30) return 'TD1_ID';
  if (lines.length >= 2 && lines[0].length === 36) return 'TD2';
  return null;
}
function parseLines(lines, type) {
  try {
    // TD3_PASSPORT → mrz-fast avec correction OCR
    ...
    // TD1_ID / TD2 → mrz (Zakodium)
    if (parseMrzGeneric) {
      const mrzLines =
        type === 'TD1_ID' ? lines.slice(0, 3) : lines.slice(0, 2);
...
function normalizeFields(f) {
  ...
  return {
    ...
    issuingState:
      (_d = f['issuingState']) !== null && _d !== void 0 ? _d : null,
    dateOfBirth: (_e = f['birthDate']) !== null && _e !== void 0 ? _e : null,
    sex: normalizeSex(f['sex']),
    expirationDate:
      (_f = f['expirationDate']) !== null && _f !== void 0 ? _f : null,
    documentNumber:
      (_g = f['documentNumber']) !== null && _g !== void 0 ? _g : null,
    personalNumber:
      (_h = f['personalNumber']) !== null && _h !== void 0 ? _h : null,
  };
}
